...
# Здесь пишем код
class PersonInfo:
    """ Класс для создания сотрудников компании """
    ...
    def short_name(self):
        """ Метод вывода короткого имени сотрудника """
    ...
    def new_salary(self):
        """ Метод расчета новой зарплаты """
        count_letter = defaultdict(int)
        for letter in ''.join(self.deps):
            count_letter[letter] += 1
        return sum(sorted(count_letter.values(), reverse=True)[:3]) * self.age * 1337
