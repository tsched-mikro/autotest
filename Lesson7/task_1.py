...
# Здесь пишем код
class Segment:
    """ Класс для работы с координатами отрезка """
    def __init__(self, a: tuple, b: tuple):
        self.a = a
        self.b = b

    def length(self):
        """ Метод расчета длины отрезка """
        return round(((self.a[0] - self.b[0])**2 + (self.a[1] - self.b[1])**2)**0.5, 2)

    def x_axis_intersection(self):
        """ Метод проверки пересечения оси абсцисс """
        return any((all((self.a[0] <= 0, self.b[0] >= 0)), all((self.a[0] >= 0, self.b[0] <= 0))))

    def y_axis_intersection(self):
        """ Метод проверки пересечения оси ординат """
        return any((all((self.a[1] <= 0, self.b[1]  >= 0)), all((self.a[1] >= 0, self.b[1] <= 0))))
